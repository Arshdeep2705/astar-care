...
| `006_retire_care_log.sql` (APPLIED 2026-09-17, REVERSED by 007 within the hour) | Archived `ac_care_logs` (policies dropped, table renamed) on a misread of the owner's instruction. Kept for an honest history; never run again. | — | — |
| `007_restore_care_log.sql` (APPLIED 2026-09-17) | Reverses 006 exactly: table renamed back, the four `cl_*` policies recreated (admin or own `worker_id`, same shape as `nm_*`/`ol_*`). The SHOWER questions are retired in the app build instead (form, Summary caption, export rows, metrics); `shower_*` columns stay for old rows. | Low. No data touched. | 58 rows readable again through the API; app build 20260917+ loads them |
