...
| `006_retire_care_log.sql` (APPLIED 2026-09-17) | Retires the personal care log on the owner's decision: drops its four RLS policies and renames `ac_care_logs` → `ac_care_logs_archive_20260917` (58 rows kept for reference, unreadable through the API). The app no longer loads, writes, exports or reports it; "Assisted transfers" on the Summary now comes only from accepted transfer observations. | Low. No data deleted; drop the archive table when it is no longer wanted. | build 20260917+ deployed first, then applied; app load verified after |
